...
app.post('/api/chatbot', async (req, res) => {
    try {
        ...
        console.error('Błąd w endpointach:', error.response?.data || error.message);
        res.status(500).json({ error: 'Wystąpił błąd w przetwarzaniu żądania', details: error.response?.data });
    }
});
